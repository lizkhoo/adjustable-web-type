...
- Anchor to the *visible* point — the control if it's a tangent, the anchor otherwise — instead of always to the displayX.
...
**Why:** Dragging the tension tangent stretched one side of the top curve only — `tTopOut` scaled with the parameter on the *outgoing* tangent from A0, but the incoming tangent at A1 used a hard-coded constant `K = 0.5523 * bowlHeight`.
