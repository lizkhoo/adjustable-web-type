...
## Brief 9 landed — code-review fixes: handle-node tracking + counter correctness (2026-05-31)

Five behavioral defects from the high-effort Brief 7/8 review, all in `AnatomyDeformWordmark` / the counter helpers. View-only/positioning fixes — no deformation-math, preset, or `toState/fromState` shape change. **Built on top of the Rubik-Bubbles rework `3a8ab81` (which landed on-branch mid-task); all edits are confined to the anatomy counter/handle code and never touch `applyBubbliness`/`smoothClosedPolyline`.** Verified live (Playwright MCP) by probing `_deformedEdgePoints` / `_computeHandlePositions` / `_resolveGlyphPath` on a fresh instance (deterministic) plus screenshots.

- **#1 `descenderDepth` node now rides the stretched descender.** `descenderDepth` is a wrap-group `scale(1 dd)` in `_glyphMarkup`, never a path-level deform, so it never reaches the sampled `cmds` — and the `_edgeCache` stateKey omitted it, so dragging it didn't even bust the cache (node frozen). Fix: in `_deformedEdgePoints`, fold `dd` into the bottom edge — `bottom.y > 0 ? bottom.y * dd : bottom.y` (only the below-baseline terminal scales; the pivot is `y=0`) — and append `dd` to the stateKey. `dd` is read only for `ANATOMY_DESCENDER_LETTERS` (1 otherwise). Verified: `bottom.y` scales exactly ×1.8 at dd=1.8 on all of g/j/p/q/y.
- **#2 + #5 single source of truth for the counter centroid.** Old bug: the handle anchored at `counterCentroid(baseCommands)` (cached `g._counterCentroid`) but `counterScale` pivoted about `counterCentroid(width/height-deformed outline)`, recomputed every frame — under non-default width/height the dot and the pivot diverged, and the recompute was wasted hot-path work. Fix: `_resolveGlyphPath` computes the pivot **once** — `counterCentroid(cmds)` after height+width when re-proportioned, else the cached base centroid — passes it into `counterScale(cmds, s, center, weight)` (new `center` param; no more internal recompute), and carries it on `result.counterPivot`. `_deformedEdgePoints` surfaces it as `edges.counter`, and the `counter` anchor branch reads that. So the dot **is** the pivot, by construction. Verified: pivot===anchor at width=1 and width=1.4 (exact), anchor x tracks 178→239.6 under width, stays fixed under counter-grow (symmetric).
- **#3 — `ANATOMY_ANCHOR_OVERRIDES` re-wired, NOT dropped (corrects the Brief 8 note above).** The Brief 8 entry claimed the `f`/`t`/`J` `height.yFrac` drops were "dead / strip them." **They are not redundant** — the live top-edge point is the hook/cap-overshoot tip, _not_ the visual top of the body, which is exactly what `yFrac` corrects. Re-wired the `top` branch to apply `yFrac` as a downward offset from the live edge (`edges.top.y + yFrac·bboxH`, so it tracks the height deform) and the `right` branch to honour `f`'s `width.xFrac` (pull in off the crossbar-extended bbox). All four keys (`f.height.yFrac`, `f.width.xFrac`, `t.height.yFrac`, `J.height.yFrac`) are now consumed — none dead. Verified: f-height drop 164.6 = 0.22·bbH, f-width hx = 0.62·bbox, t-height drop 198 = 0.3·bbH.
- **#4 serif-foot band is dilation-aware.** Replaced the independent `±0.12·spanY` foot band with the single Brief 3c baseline tolerance `0.10·xHeight` (from `_bandMetrics`), widened downward by `weight/2`: `y ∈ [−tol, tol + weight/2]`. Dilation pushes feet below the baseline by half the stroke, so a strict `|y|≤tol` band loses them under heavy weight. Verified: foot on `d`/`t` stays in-band and tracks the terminal at weight=36 + serif=1.6.
- **#5 counter growth cap is gap-based + weight-aware.** The old cap only checked the four axis-aligned bbox extents, so a diagonally-widest bowl (`a`/`e`) could poke through between axes, and it ignored `weight`. New cap (`counterScale`, `s>1`): sample the outer + counter contours densely, and for each counter sample take its min gap to the outer contour and its radius `r` from the pivot — by the triangle inequality the post-scale gap is ≥ `gap − (s−1)·r`, so cap `s ≤ 1 + (gap − floor)/r` per sample. `floor = COUNTER_STROKE_FLOOR_FRAC·min(outerDim) + weight/2`. Verified: at counter=1.4 (max) the scaled counter bbox stays strictly inside the outer with positive margins on all sides (`a`, `e`); `o` at weight=40 + counter=1.4 keeps a perfect ring.

**Non-obvious gotcha — heavy-weight bowl fragmentation is NOT a counter breach.** At `weight≈40` on the cursive Instrument Serif `a`/`e`, the _outer_ outline self-fragments (white gashes) — but this is a pre-existing `dilateOutline` self-intersection limitation, present identically at `counterContour=1.0` (counter scaling not even called). Proven by rendering c=1.0 vs c=1.4 side-by-side (identical fragmentation) and a clean `o` ring at the same weight. Out of Brief 9 scope (don't touch the deform math); the counter ring invariant itself holds.

**Verification.** Fresh-instance probes (deterministic; the live demo's combined preset+text change races a transient partial render — settle ~1.5s or drive a fresh instance). `gjpqy Bodega`: 55 handles, counterContour on g,p,q,B,o,d,e,g,a (Brief 7 DoD set), descenderDepth on g,j,p,q,y, `g` carries both. `toState→fromState` path data byte-identical with counter/descender/width/height/serif/weight all set; `toSVG()` works; `f` (no counter) → `counterPivot:null` (handle dropped). Zero console errors beyond favicon. `node --check` + prettier clean.

## Brief 10 landed — code-review cleanup: dedup, hot-path sampling, whitelist, dead code (2026-05-31)

Quality-only cleanup on top of Brief 9. **Zero behavior change**, proven by a before/after Playwright snapshot: across all 4 anatomy presets × {`Hello jazz`, `Bodega`}, every glyph's path `d` string and every handle `cx`/`cy`/cursor is **byte-identical** (0 path diffs, 0 handle diffs, 0 handle-set changes). Built on top of the Rubik-Bubbles rework; never touches the bubbly engine.

- **#6 — `ANATOMY_COUNTER_LETTERS` whitelist replaced by the runtime detector.** Deleted the hand-curated 14-glyph list + `COUNTER_CONTOUR_LETTER_HANDLES` + the apologetic "B was dropped" comment. The counterContour handle is now wired onto any glyph for which `counterCentroid(baseCommands)` is non-null, gated by a preset-level `counterContour: true` flag (contrast/serif/sans opt in; mono/bubble omit it). New `AnatomyDeformWordmark.presetHasCounterContour` getter + `_glyphHasCounterHandle(baseCommands)` predicate; `anatomyHandleIdsFor`'s old `letterHandles` map param became an `addCounter` boolean; `_defaultHandleStateFor` now takes `baseCommands` (threaded at all 5 call sites: `_loadGlyphs`, the `fromState` restore loop, `setPresetKey`, `resetAll`, `resetGlyph`). The `presetLetterHandles` getter is gone (counterContour was its only consumer). **Glyphs the detector now counters that the old whitelist missed: `A`, `0`, `6`, `8`, `9`** (all genuinely have an enclosed counter — A's apex triangle, the bowls/loops of the figures). None appear in the demo strings, so the snapshot is unaffected; they only surface when those characters are typed. Detector loses none of the old 14.
- **#8 — `_deformedEdgePoints` sampling cut from 3 passes to 2.** The bounds (min/max) accumulation is now folded into the same loop that builds the sample cloud, instead of a second pass over `pts`. Edge points are byte-identical (same samples, same bounds, `samplesPerCurve` left at 12 — lowering it would move the points). The dragged glyph misses the cache every frame of its own drag, so this is the hot path.
- **#9 — one `detectCounters(subs)` helper.** `counterSubpathIndices`, `counterCentroid`, `counterScale`, and `anatomyWidth`'s `hasCounter` each independently re-walked every subpath running `signedAreaOfCommands`; `counterScale` also re-found the outer contour that the index scan already had. Extracted `detectCounters(subs) → { outerIndex, counterIndices, centroid, areas }` computing the split + per-subpath signed areas **once**. `counterSubpathIndices`/`counterCentroid` are now thin wrappers; `counterScale` takes `outerIndex`/`centroid`/`counterIndices` straight from one detector pass (dropped its private `outerI` loop); `anatomyWidth` uses `detectCounters(subs).counterIndices.length > 0` (the gate its JSDoc always claimed it shared). Same winding + 2%-area + `|outerArea|<1` rule as before → identical results for real glyphs.
