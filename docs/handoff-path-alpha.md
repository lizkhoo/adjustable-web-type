> **Superseded — kept for history.** Path α has shipped as `AnatomyDeformWordmark` in `lib/sculpt.js`. The canonical record of the decision is [ADR 0001](adr/0001-per-preset-pipeline-routing.md); the public surface is in [API.md](API.md). This doc remains as the record of the gesture model the now-deleted prototype validated.

# Handoff — Path α (per-letter anatomy handles on the WOFF outline)

**Status:** Superseded — shipped. Prototype validated the gesture model; production landed via Briefs 1–10 (see [docs/briefs/path-alpha-landing.md](briefs/path-alpha-landing.md)).
**Date:** 2026-05-27 (shipped 2026-06-01).
**Predecessor:** [`handoff-pipeline-prototype.md`](handoff-pipeline-prototype.md) — the doc that drove the prototype.
**Prototype:** a throwaway demo page (since deleted) — the gesture model it validated is recorded below.

---

## The feature, in one sentence

Picking a font preset should mount the **real WOFF outline** as the starting shape, with **per-letter drag handles** anchored on each glyph that the user can grab to deform individual letters in place — _except_ for Rubik Bubbles, which keeps the existing single-axis outline-deform pipeline.
...
| `ibmPlexMono`     | height, width, weight              | Mono cell is the default starting position; dragging width breaks the cell visually (see _Open questions_ below). |
...
The prototype mounts `SculptLettering.DeformableOutlineWordmark` (so the WOFF outline is the starting shape), then attaches an SVG overlay layer with per-glyph circles wired to drag events. Each drag updates the per-glyph SVG `transform` or stroke attributes — _the underlying path data is never modified_.
...
| `width`       | `scale(sx, 1)`, pivoted at local x = bbox.x (glyph's left edge). Scales bowls _and_ stems uniformly.                                                          | Anatomy-aware partitioning: scale only the bowl/counter regions, preserve stem thickness. Requires identifying stem vs. bowl on the outline.                                                                                                    |
...
The prototype does _not_ modify `lib/sculpt.js`. To ship Path α for real:
...
2. **Descender depth.** ✅ **Yes — add a `descenderDepth` handle on the five descender letters** (`g`, `j`, `p`, `q`, `y`). Anchored bottom-center, scales only the below-baseline portion. Same affine-transform primitive as the other handles. Surfaces _descender_ as a teaching concept.
...
- The throwaway prototype page — source of truth for the gesture model during the prototype session; deleted in Brief 4 after production landed α (gesture model absorbed into this doc).
