...
(function (root, factory) {
  ...
  const bubbly = {
    name: 'bubbly',
    defaults: {           // ← NEW: applied to every glyph not in glyphParams
      strokeWeight: 28,
      xHeight: 140,
      capHeight: 168,
      curvature: 0.7,     // monoline glyphs pick up a sympathetic round body
      slant: 0,
    },
    glyphParams: {
  ...
  class Wordmark {
      ...
      if (this.interactive) {
        ...
        placed.forEach((pg, idx) => {
          ...
          for (const h of hs) {
            ...
            if (showArm) {
              html.push(`<line x1="${anchorX}" y1="${anchorY}" x2="${controlX}" y2="${controlY}" stroke="#d24d3a" stroke-width="1" opacity="0.55" pointer-events="none"/>`);
              html.push(`<rect x="${anchorX - 2.5}" y="${anchorY - 2.5}" width="5" height="5" fill="#d24d3a" pointer-events="none"/>`);
            }
            const fill = h.isTangent ? '#ffe6e0' : 'white';
            ...
            html.push(`<circle cx="${displayX}" cy="${displayY}" r="${controlRadius}" fill="${fill}" stroke="#d24d3a" stroke-width="1.5" pointer-events="none"/>`);
          }
        });
    ...
    _renderTooltip(totalWidth, height) {
      ...
      // Bubble: light cream fill, thin accent border, no shadow.
      parts.push(`<rect x="${bx}" y="${by}" width="${bubbleW}" height="${bubbleH}" rx="3" ry="3" fill="#fffdf8" stroke="#d24d3a" stroke-width="1"/>`);
      // Label (param name) in muted color
      parts.push(`<text x="${bx + pad}" y="${by + 15}" fill="#7a5f51" font-size="11.5" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" letter-spacing="0.02em">${this._esc(labelText)}</text>`);
      // Value in accent
      parts.push(`<text x="${bx + pad + labelW + gap}" y="${by + 15}" fill="#d24d3a" font-size="11.5" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-weight="600">${this._esc(valueText)}</text>`);
      parts.push('</g>');
  ...
  if (state.modes && state.modes.mouseFollow) {
    wm.enableMouseFollow(state.modes.mouseFollow.opts);
      ...
      return [
        ...
        '<style>',
        '  html, body { margin: 0; height: 100%; background: #faf8f3; display: flex; align-items: center; justify-content: center; }',
