...
(function (root, factory) {
  ...
  function applyBubbliness(commands, t, _bounds, opts) {
    ...
    // Brief 8 A1: BUMPS_MAX raised 20→24 so the 0.5→max half of the
    // (now-centered) slider adds visibly more bumps over native.
    const BUMPS_MAX = 24;
    ...
    const ampNorm =
      opts && opts.amplitude != null
        ...
        : 0.5;
    // Brief 8 A2: coefficient 0.12→0.22 (~1.8×) so sweeping the Amplitude
    // slider min→max is an obvious change in bump height. ampNorm=0 still
    // flattens synthetic bumps to nothing.
    const amplitude = glyphSize * 0.22 * ampNorm * Math.sqrt(t);
  ...
  /**
   * Brief 7 — counter contour. Identify a glyph's enclosed counter(s): the
   * subpath(s) wound opposite the largest (outer) contour with non-trivial
   * area. Uses the same gate as anatomyWidth's `hasCounter` so the two agree
   * on which glyphs have a counter worth manipulating. Returns the indices
   * (into splitSubpaths order) of the counter subpaths.
   */
  function counterSubpathIndices(subs) {
    ...
    if (Math.abs(outerArea) < 1) return [];
    const outerSign = outerArea >= 0 ? 1 : -1;
    const idx = [];
  ...
  function counterCentroid(commands) {
    if (!commands) return null;
    const subs = splitSubpaths(commands);
    const idx = counterSubpathIndices(subs);
    if (!idx.length) return null;
    let best = idx[0];
    let bestA = 0;
    for (const i of idx) {
      const a = Math.abs(signedAreaOfCommands(subs[i].commands));
      if (a > bestA) {
        bestA = a;
        best = i;
      }
   ...
   */
  function counterScale(commands, s) {
    if (!commands || Math.abs(s - 1) < 0.001) return commands;
    const subs = splitSubpaths(commands);
    const cidx = counterSubpathIndices(subs);
    if (!cidx.length) return commands;
    const c = counterCentroid(commands);
    if (!c) return commands;

    let sEff = s;
    if (s > 1) {
      // Outer contour = largest-|area| subpath; combined bounds of every
      // counter subpath (B/g have two). Cap so each counter edge stays
      // `floor` inside the matching outer edge.
      let outerI = 0;
      let outerA = 0;
      for (let i = 0; i < subs.length; i++) {
        const a = Math.abs(signedAreaOfCommands(subs[i].commands));
      ...
      let kMinX = Infinity,
        ...
        kMaxY = -Infinity;
      for (const i of cidx) {
        const b = boundsFromCommands(subs[i].commands);
        if (b.minX < kMinX) kMinX = b.minX;
        if (b.maxX > kMaxX) kMaxX = b.maxX;
        if (b.minY < kMinY) kMinY = b.minY;
        if (b.maxY > kMaxY) kMaxY = b.maxY;
      }
      const floor =
        COUNTER_STROKE_FLOOR_FRAC *
        Math.min(ob.maxX - ob.minX, ob.maxY - ob.minY);
      const caps = [];
      if (kMaxX > c.x) caps.push((ob.maxX - floor - c.x) / (kMaxX - c.x));
      if (c.x > kMinX) caps.push((c.x - floor - ob.minX) / (c.x - kMinX));
      if (kMaxY > c.y) caps.push((ob.maxY - floor - c.y) / (kMaxY - c.y));
      if (c.y > kMinY) caps.push((c.y - floor - ob.minY) / (c.y - kMinY));
      const fit = caps.length ? Math.min.apply(null, caps) : Infinity;
      if (isFinite(fit)) sEff = Math.min(s, Math.max(1, fit));
    }
  ...
  function anatomyHandleIdsFor(character, presetHandles, letterHandles) {
    ...
    // Brief 7 — per-letter additions on top of the preset baseline (add-to
    // semantics: a letter never has fewer handles than the preset declares).
    // Used to reach `counterContour` onto the letters that have a counter
    // worth showing. Dedup via `seen` so an override that repeats a baseline
    // id (or descenderDepth below) is harmless.
    if (letterHandles) {
      const extra = letterHandles[character];
      if (extra) for (const id of extra) push(id);
    }
  ...
  // Brief 7 — letters that earn a `counterContour` handle on the three
  // contrast / serif / sans presets (instrumentSerif, bitter, sourceSans).
  // Each has an enclosed counter worth manipulating pedagogically. Mono
  // (ibmPlexMono) and bubble (bubbly) opt out — see the preset wiring.
  //
  // `B` is included beyond the brief's enumerated list: the brief's stated
  // criterion is "anything with an enclosed inner region" (B has two counters)
  // and its own verification step expects a counter handle on B ("type Bodega
  // … counter handles on B, o, d, e"). The enumeration simply dropped it.
  const ANATOMY_COUNTER_LETTERS = [
    "O",
    "b",
    "d",
    "e",
  ...
  const COUNTER_CONTOUR_LETTER_HANDLES = (function () {
    const m = {};
    for (const ch of ANATOMY_COUNTER_LETTERS) m[ch] = ["counterContour"];
    return m;
  })();
  ...
  const bubbly = {
    ...
    axes: [
      // Brief 8 A1: default rests at center (0.5) so the slider is
      // bidirectional — left reduces synthetic bumps toward the native
      // outline floor (bubbliness=0 = raw font, never below), right adds
      // more (up to BUMPS_MAX). The native Rubik Bubbles outline is
      // already bubbly; this axis is additive on top of it.
  ...
  const instrumentSerif = {
    ...
    letterHandles: COUNTER_CONTOUR_LETTER_HANDLES, // Brief 7 — counter on O b d e o p q g a D P Q R
    defaults: {
      strokeWeight: 22,
      xHeight: 138,
      capHeight: 166,
...
(function () {
  ...
  class AnatomyDeformWordmark {
    ...
    // Brief 7 — per-letter handle additions for the active preset, or null.
    // `{ [character]: [...handleIds] }`; added on top of the preset baseline.
    get presetLetterHandles() {
      const p = this.presetMeta;
      return (p && p.letterHandles) || null;
    }
    ...
    _resolveGlyphPath(g) {
      ...
      if (
        ...
        return { d: g.pathData, strokeAttrs: "", cmds: g.baseCommands || null };
      }
      ...
      try {
        ...
        if (counterActive) {
          ...
          cmds = counterScale(cmds, counter);
        }
        if (serifActive) {
        ...
        if (weight > 0) {
          ...
          if (dilated && dilated.length) {
            // cmds carries the dilated outline so the weight/left handle
            // node (Brief 8 B1) tracks the thickened silhouette edge.
            result = {
              d: commandsToPathData(dilated),
              strokeAttrs: "",
              cmds: dilated,
            };
          } else if (
            ...
            result = { d: commandsToPathData(cmds), strokeAttrs: "", cmds };
          }
        } else {
          result = { d: commandsToPathData(cmds), strokeAttrs: "", cmds };
        }
    ...
    _deformedEdgePoints(g) {
      ...
      let right = null,
        ...
        bottom = null,
        foot = null;
      const footTol = Math.max(spanY * 0.12, 1);
      for (const p of pts) {
        ...
        // Baseline foot: y within tol of 0 (outline baseline), pick rightmost.
        if (Math.abs(p.y) <= footTol && (foot === null || p.x > foot.x)) {
          foot = p;
        }
      }
      // Fallbacks if a band caught nothing (degenerate glyph).
      if (!right) right = { x: maxX, y: midY };
      if (!left) left = { x: minX, y: midY };
      if (!top) top = { x: midX, y: minY };
      if (!bottom) bottom = { x: midX, y: maxY };
      if (!foot) foot = { x: maxX, y: Math.min(0, maxY) };

      const edges = { right, left, top, bottom, foot, minX, maxX, minY, maxY };
    ...
    _computeHandlePositions(layout, baselineY) {
      ...
      const letterHandles = this.presetLetterHandles;
      const out = [];
      for (let i = 0; i < layout.placed.length; i++) {
        ...
        // Falls back to bbox-derived anchors when sampling degenerates.
        const edges = this._deformedEdgePoints(g);
        ...
        for (const id of ids) {
          ...
          if (spec.anchor === "top") {
            ...
            // (ascender / cap / x-height as appropriate). The `f`/`t`/`J`
            // yFrac overrides are now redundant (the real edge supplies y);
            // an xFrac override still nudges the node along the top edge.
            if (edges) {
              ...
              hx =
                ...
                (xFrac != null
                  ...
                  : edges.top.x);
              hy = baselineY + edges.top.y - NUDGE;
            } else {
            ...
            if (edges) {
              hx = pg.x + edges.right.x + NUDGE;
              hy = baselineY + edges.right.y;
            } else {
              hx = pg.x + bb.maxX + 18;
              hy = baselineY + (bb.minY + bb.maxY) / 2;
            }
            cursor = "ew-resize";
            ...
            if (edges) {
              ...
              hx = pg.x + (bb.minX + bb.maxX) / 2;
              hy = baselineY + bb.maxY + 22;
            }
            cursor = "ns-resize";
          } else if (spec.anchor === "counter") {
            // Brief 7 — anchor on the centroid of the glyph's largest
            // enclosed counter, derived from the base outline (cached per
            // glyph — baseCommands never change). A letter whose counter the
            // detector can't find drops the handle entirely.
            const c =
