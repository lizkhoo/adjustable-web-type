...
(function () {
  ...
  class AnatomyDeformWordmark {
    ...
    // serifLength sits between the two: the feet pin at y≈0 (which height
    // already fixed), and weight should dilate the already-extended foot
    // uniformly. width remains a CSS affine on the wrap group (see
    // _transformFor) and doesn't participate here.
    //
    // Result is memoised on the glyph keyed by (height, serifLength, weight)
    ...
    _resolveGlyphPath(g) {
      ...
      if (weight <= 0 && !heightActive && !serifActive) {
        return { d: g.pathData, strokeAttrs: "" };
      }

      // Memo: reuse if height, serifLength and weight all match last compute.
      if (
        g._pathCache &&
      ...
      if (!g.baseCommands) {
        g._pathCache = { weight, height, serif, result: strokeFallback };
      ...
      try {
        ...
        if (weight > 0) {
          ...
          } else if (heightActive || serifActive) {
            // Dilation degenerated but the height/serif edits are still valid.
      ...
      g._pathCache = { weight, height, serif, result };
      return result;
    }
    ...
    // is `translate(left 0) scale(sx 1) translate(-left 0)`, pivoted at the
    // glyph's left edge (bounds.minX).
    _transformFor(glyph) {
      ...
      // _resolveGlyphPath). `serifLength` is likewise path-level as of Brief 3c
      // (a serif-foot translation, also in _resolveGlyphPath). Only `width`
      // remains as a wrap-group affine (still prototype-grade, pending
      // Brief 3d).
      let sx = 1;
