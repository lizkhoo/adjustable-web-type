...
(function (root, factory) {
  ...
  // CSS for the exported-bundle attribution tooltip (see outlineAttributionHtml).
  // A muted "Attribution" label below the wordmark; the disclaimer + credit
  // reveal on hover/focus. Single <style> line — pure CSS, no JS in the export.
  const OUTLINE_ATTRIBUTION_CSS =
    "  .sculpt-attribution { position: relative; display: flex; justify-content: center; margin-top: 16px; }" +
    " .sculpt-attribution__label { font: inherit; font-size: 12px; color: #8b929e; background: none; border: 0; padding: 4px 8px; cursor: help; text-decoration: underline dotted; text-underline-offset: 3px; }" +
    " .sculpt-attribution__tip { position: absolute; top: 100%; left: 50%; transform: translateX(-50%); margin-top: 6px; width: max-content; max-width: 320px; display: none; flex-direction: column; gap: 6px; padding: 10px 12px; font-size: 12px; line-height: 1.5; text-align: left; color: #525860; background: #fff; border: 1px solid #d4d8e0; border-radius: 4px; box-shadow: 0 4px 14px rgba(0, 0, 0, 0.08); z-index: 10; }" +
    " .sculpt-attribution:hover .sculpt-attribution__tip, .sculpt-attribution:focus-within .sculpt-attribution__tip { display: flex; }" +
    " .sculpt-attribution__credit { color: #8b929e; }";
  ...
  function outlineAttributionHtml(presetKey) {
    const p = presets[presetKey];
    if (!p || !p.attribution) return "";
    // Rendered below the wordmark as an "Attribution" label whose full
    // disclaimer + credit reveals on hover/focus (see .sculpt-attribution CSS
    // in the bundle builders). Pure CSS tooltip — no JS needed in the export.
    return (
      '<div class="sculpt-attribution" role="note">' +
      '<button type="button" class="sculpt-attribution__label" ' +
      'aria-describedby="sculpt-attribution-tip">Attribution</button>' +
      '<span class="sculpt-attribution__tip" id="sculpt-attribution-tip" ' +
      'role="tooltip">' +
      '<span class="sculpt-attribution__note">' +
      OUTLINE_DISCLAIMER_SHORT +
      "</span>" +
      '<span class="sculpt-attribution__credit">' +
      p.attribution +
      " (" +
      p.license +
      ")</span>" +
...
(function () {
  ...
  class DeformableOutlineWordmark {
    ...
    disableMouseFollow() {
      ...
      // Restore the rest values onto every glyph too (mouse-follow broadcasts
      // per-glyph), so letters don't stay stuck at the last cursor position.
      for (const g of this.glyphs) {
        g.axisValues = Object.assign({}, this.axisValues);
      }
      this._mouseFollow = null;
    ...
    _applyMouseFollow(dx, dy) {
      ...
      const apply = (ax, delta) => {
        ...
        // Mouse-follow is a master gesture: broadcast to every glyph, since
        // _applyDeformToGlyphs reads each glyph's own axisValues (which is
        // always seeded). Without this the cursor would deform nothing.
        for (const g of this.glyphs) {
          if (g.axisValues) g.axisValues[ax.id] = t;
        }
      };
    ...
    // Per-glyph axis overrides that differ from the wordmark-level axisValues.
    // Only the diff is serialized (keeps exported bundles small); an empty
    // object means the glyph tracks the master values.
    _glyphAxisDiff(g) {
      const diff = {};
      if (!g.axisValues) return diff;
      for (const ax of this.presetAxes) {
        const v = g.axisValues[ax.id];
        if (v != null && v !== this.axisValues[ax.id]) diff[ax.id] = v;
      }
      return diff;
    }
    ...
    static async fromState(state) {
      ...
      // Restore per-letter axis overrides (drag-on-glyph) by index, so the
      // exported/reloaded wordmark matches the per-letter look. Older states
      // without axisDiff simply track the master values.
