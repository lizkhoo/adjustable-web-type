  ...
  class AnatomyDeformWordmark {
    ...
    _glyphMarkup(pg, i, opts) {
      ...
      if (!isDescender || dd == null) {
        const pathMarkup = `<path d="${g.pathData}" fill="${this.color}" fill-rule="evenodd"${strokeAttrs}/>`;
        return (
          `<g transform="translate(${pg.x},${o.baselineY})" data-glyph-idx="${i}">` +
      ...
      const defs =
        ...
        `</clipPath>` +
        `</defs>`;
      const pathTag = `<path d="${g.pathData}" fill="${this.color}" fill-rule="evenodd"${strokeAttrs}/>`;
    ...
    _render() {
      ...
      // Weight is a stroke overlay (paint-order: stroke fill). Descender
      // letters split into two clipped layers — see _glyphMarkup.
      const parts = [];
    ...
    _renderHandles(layout, baselineY) {
      ...
      for (let i = 0; i < layout.placed.length; i++) {
        ...
        for (const id of ids) {
          ...
          let hx, hy, cursor;
          if (spec.anchor === "top") {
            // Default: cx (bbox center), top - 18px
            // Override: yFrac shifts the anchor down by that fraction of
            // bbox height; xFrac biases left/right.
            ...
            // descenderDepth: bottom-center of bbox, 22px below.
            const xFrac = ov && ov.xFrac != null ? ov.xFrac : 0.5;
            const yFrac = ov && ov.yFrac != null ? ov.yFrac : 1;
            hx = pg.x + bb.minX + bw * xFrac;
    ...
    _onHandleMove(ev, drag) {
      ...
      if (drag.handleId === "height") {
        ...
        v = clampNum(drag.startV + dy / 80, 0.1, 2.4);
      } else {
        return;
      }
