...
(function (root, factory) {
  ...
  // intersects preset.handles with letter-level skip rules (round-bottom serif).
...
(function () {
  ...
  class DeformableOutlineWordmark {
    ...
    _applyMouseFollow(dx) {
      ...
      const t = Math.max(
        ax.min,
        Math.min(ax.max, rest + (dx / Math.max(1, o.clamp)) * span * 0.5)
      );
...
(function () {
  ...
  //
  // Per-glyph transforms are prototype-grade (per-glyph affine on a wrap
  // group, plus an SVG stroke overlay for `weight`). Production-grade
  // math — outline dilation, region-clipped scale, anchor-point
  // translation, anatomy-aware partitioning — is Brief 3.
  //
  ...
  class AnatomyDeformWordmark {
    constructor(text, options) {
      ...
      // height/width/serifLength are multiplicative scales (default 1);
      // weight is an additive stroke width in user units (default 0).
    ...
    _render() {
      ...
      // Weight is a stroke overlay (paint-order: stroke fill).
      const parts = [];
      for (let i = 0; i < placed.length; i++) {
        ...
        const strokeAttrs =
          weight > 0
            ...
            : "";
        const pathMarkup = g.pathData
          ? `<path d="${g.pathData}" fill="${this.color}" fill-rule="evenodd"${strokeAttrs}/>`
          : "";
        parts.push(
          `<g transform="translate(${pg.x},${baselineY})" data-glyph-idx="${i}">` +
    ...
    _renderHandles(layout, baselineY) {
      ...
      for (let i = 0; i < layout.placed.length; i++) {
        ...
        // Anchor positions are derived from the (un-transformed) bbox in
        // local glyph coordinates, then offset to wordmark coordinates.
        const bb = g.bounds;
        const left = pg.x + bb.minX;
        const right = pg.x + bb.maxX;
        const cx = pg.x + (bb.minX + bb.maxX) / 2;
        const top = baselineY + bb.minY;
        const midY = baselineY + (bb.minY + bb.maxY) / 2;
        const bottom = baselineY + bb.maxY;
        ...
        for (const id of ids) {
          ...
          html.push(
            `<circle data-glyph-idx="${i}" data-handle-id="${id}" cx="${hx}" cy="${hy}" r="${HANDLE_R}" fill="#ffffff" stroke="${STEM}" stroke-width="1.5" cursor="${cursor}" style="touch-action:none">` +
              `<title>${this._esc(spec.title)}</title>` +
              `</circle>`
          );
        }
      }
    ...
    toSVG() {
      ...
      const body = placed
        .map((pg, i) => {
          ...
          return (
            `<g transform="translate(${pg.x},${baselineY})" data-glyph-idx="${i}">` +
            `<g${tr ? ` transform="${tr}"` : ""}>${pathMarkup}</g>` +
            `</g>`
          );
        })
        .join("");
