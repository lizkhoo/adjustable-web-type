...
(function () {
  ...
  //
  // Per-glyph transforms are mostly prototype-grade (per-glyph affine on a
  // wrap group). `weight` is production-grade as of Brief 3a — a real
  // polyline offset-path dilation (see dilateOutline / _weightedPath), with
  // the old SVG stroke overlay kept as a per-glyph fallback. The remaining
  // production-math upgrades — region-clipped scale, anchor-point
  // translation, anatomy-aware partitioning — are Brief 3b–3d.
  //
  ...
  class AnatomyDeformWordmark {
    ...
    // Resolve the fill path + stroke attributes for one glyph, applying the
    // `weight` handle as a real outline dilation (Brief 3a). The silhouette
    // grows from the inside-out via a polyline offset path; counters shrink.
    // Result is memoised on the glyph keyed by the weight value so dragging
    // other handles doesn't recompute the offset every frame.
    //
    // Falls back to the original SVG stroke overlay (paint-order: stroke
    // fill) when the glyph has no source outline commands or the offset math
    // produces a degenerate result.
    _weightedPath(g) {
      const weight = g.handleState.weight || 0;
      if (weight <= 0 || !g.pathData) {
        return { d: g.pathData, strokeAttrs: "" };
      }
      const strokeFallback = {
        d: g.pathData,
        strokeAttrs: ` stroke="${this.color}" stroke-width="${weight}" paint-order="stroke fill" stroke-linejoin="round" stroke-linecap="round"`,
      };
      if (!g.baseCommands) return strokeFallback;

      // Memo: reuse if the weight hasn't changed since last compute.
      if (g._weightCache && g._weightCache.weight === weight) {
        return g._weightCache.result;
      }

      let result = strokeFallback;
      try {
        // delta = weight/2: a centered stroke of width W grew the outer
        // silhouette by W/2, so offsetting each contour outward by W/2 keeps
        // the same outer growth the drag/mouse-follow ranges were tuned for.
        const dilated = dilateOutline(g.baseCommands, weight / 2);
    ...
    _glyphMarkup(pg, i, opts) {
      ...
      const rendered = this._weightedPath(g);
      ...
      const bb = g.bounds;
      const padX = 80; // pad so wide weight strokes / scaled widths stay clipped
      const padY = 80;
    ...
    _render() {
      ...
      // Weight is a real outline dilation (offset path) per Brief 3a, with a
      // stroke overlay fallback — see _weightedPath. Descender letters split
    ...
    // Build the per-glyph transform string from handleState. The transform
    // is `translate(left 0) scale(sx sy) translate(-left 0)`, pivoted at the
    // glyph's left edge (bounds.minX). `serifLength` multiplies sx the same
    // way `width` does (per the prototype) — only the anchor differs.
    _transformFor(glyph) {
      const s = glyph.handleState;
      let sx = 1,
        sy = 1;
      if (s.width != null) sx *= s.width;
      if (s.serifLength != null) sx *= s.serifLength;
      if (s.height != null) sy *= s.height;
      if (sx === 1 && sy === 1) return "";
      const left = glyph.bounds.minX;
      // translate(left 0) scale(sx sy) translate(-left 0) pivots at x=left, y=0.
      return `translate(${left} 0) scale(${sx} ${sy}) translate(${-left} 0)`;
    }
