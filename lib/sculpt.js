    ...
    _renderHandles(layout, baselineY) {
      ...
      for (let i = 0; i < layout.placed.length; i++) {
        ...
        for (const id of ids) {
          ...
          html.push(
            `<circle data-glyph-idx="${i}" data-handle-id="${id}" data-hit-area="true" cx="${hx}" cy="${hy}" r="${HIT_R}" fill="transparent" stroke="none" pointer-events="all" cursor="${cursor}" style="touch-action:none"/>` +
              `<circle cx="${hx}" cy="${hy}" r="${HANDLE_R}" fill="#ffffff" stroke="${STEM}" stroke-width="1.5" pointer-events="none"/>`
          );
        }
      }
      this.handleLayer.innerHTML = html.join("");
