...
(function () {
  ...
  //
  // Per-glyph transforms are part production-grade, part prototype-grade
  // (see _resolveGlyphPath / _transformFor). `weight` is a real polyline
  // offset-path dilation (Brief 3a) and `height` is a region-clipped vertical
  // scale (Brief 3b), both baked into the glyph commands; the old SVG stroke
  // overlay is kept as a per-glyph `weight` fallback. `width`/`serifLength`
  // remain a wrap-group affine pending Briefs 3c/3d.
  //
  ...
  class AnatomyDeformWordmark {
    ...
    // scale (Brief 3b), then `weight` as a real outline dilation (Brief 3a).
    //
    ...
    // width/serifLength remain a CSS affine on the wrap group (see
    // _transformFor) and don't participate here.
    //
    // Result is memoised on the glyph keyed by (height, weight) so dragging an
    // unrelated handle doesn't recompute the offset every frame. Falls back to
    // the SVG stroke overlay (paint-order: stroke fill) for `weight` when the
    // glyph has no source outline commands or the offset math degenerates.
    _resolveGlyphPath(g) {
      ...
      if (weight <= 0 && !heightActive) {
        return { d: g.pathData, strokeAttrs: "" };
      }

      // Memo: reuse if neither height nor weight changed since last compute.
      if (
        g._pathCache &&
      ...
      if (!g.baseCommands) {
        g._pathCache = { weight, height, result: strokeFallback };
      ...
      try {
        ...
        if (weight > 0) {
          ...
          if (dilated && dilated.length) {
            ...
            // Dilation degenerated but the height scale is still valid.
      ...
      g._pathCache = { weight, height, result };
      return result;
    }
    ...
    _transformFor(glyph) {
      ...
      // `height` is no longer a CSS scale — as of Brief 3b it's a path-level
      // region-clipped vertical scale baked into the glyph commands (see
      // _resolveGlyphPath). Only width/serifLength remain as a wrap-group
      // affine (still prototype-grade, pending Briefs 3c/3d).
      let sx = 1;
      if (s.width != null) sx *= s.width;
      if (s.serifLength != null) sx *= s.serifLength;
      if (sx === 1) return "";
      const left = glyph.bounds.minX;
      // translate(left 0) scale(sx 1) translate(-left 0) pivots at x=left.
      return `translate(${left} 0) scale(${sx} 1) translate(${-left} 0)`;
    }
