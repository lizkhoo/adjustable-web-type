...
(function (root, factory) {
  ...
  const a = (function () {
    const defaultParams = {
      ...
      aperture: 16, terminalLength: 20, bowlTopTension: 0.5523, bowlBottomTension: 0.5523,
      terminalArm: 12, serifLength: 0, leftBearing: 0, rightBearing: 0,
    };
    const paramRanges = {
    ...
    function handles(p) {
      ...
      const list = [
        ...
        { id:'bowlBottomTension', anchor:{x:g.bowlCenterX,y:g.bowlBottom}, control:{x:g.bowlCenterX+p.bowlWidth*p.bowlBottomTension,y:g.bowlBottom},
          paramName:'bowlBottomTension', deltaFromDrag:(dx)=>dx/Math.max(1,p.bowlWidth), isTangent:true },
      ...
      if (p.serifLength > 0) {
        list.push({ id:'serifLength', anchor:{x:g.stemX,y:0}, control:{x:g.stemX+p.serifLength,y:0},
          paramName:'serifLength', deltaFromDrag:(dx)=>dx });
      }
      return list;
    }
    function bounds(p) {
    ...
    function baseAdvance(p) {
      return p.strokeWeight + 2 * p.bowlWidth + p.strokeWeight / 2 + Math.max(0, p.terminalLength);
    }
    return {
  ...
  function makeArchGlyph(character, opts) {
    ...
    function g(p) {
      ...
      const stemTopY = opts.hasAscender ? xLineY - p.ascenderRise - overshoot : xLineY;
  ...
  const e = (function () {
    const dp = {
      bowlWidth:58, bowlHeight:58, strokeWeight:24, aperture:28, crossbarOffset:10,
      bowlTopTension:0.5523, bowlSideTension:0.5523, serifLength:0, leftBearing:0, rightBearing:0,
    };
    const r = {
      bowlWidth:{min:25,max:90}, bowlHeight:{min:25,max:90}, strokeWeight:{min:6,max:44},
      aperture:{min:6,max:60}, crossbarOffset:{min:-20,max:20},
      bowlTopTension:{min:0.15,max:1.4}, bowlSideTension:{min:0.15,max:1.4},
      serifLength: SERIF_RANGE, leftBearing: BEARING_RANGE, rightBearing: BEARING_RANGE,
    };
    function g(p) {
      const hs = p.strokeWeight/2, cx = hs+p.bowlWidth, cy = -p.bowlHeight;
      const aperture = clampAperture(p.aperture, p.strokeWeight, p.bowlHeight);
      return { hs, cx, cy, aperture,
    ...
    function construct(p) {
      ...
      paths.push(`M ${gg.left.x} ${gg.crossbarY} L ${topOpen.x} ${gg.crossbarY}`);
      if (p.serifLength > 0) {
        appendSerifStubs(paths, [
    ...
    function handles(p) {
      ...
      const list = [
        ...
        {id:'aperture', anchor:{x:gg.cx+p.bowlWidth,y:gg.crossbarY}, control:{x:gg.cx+p.bowlWidth,y:gg.crossbarY+gg.aperture}, paramName:'aperture', deltaFromDrag:(_,dy)=>dy},
        {id:'bowlTopTension', anchor:gg.top, control:{x:gg.top.x+tw,y:gg.top.y}, paramName:'bowlTopTension', deltaFromDrag:(dx)=>dx/Math.max(1,p.bowlWidth), isTangent:true},
        {id:'bowlSideTension', anchor:gg.left, control:{x:gg.left.x,y:gg.left.y+th}, paramName:'bowlSideTension', deltaFromDrag:(_,dy)=>dy/Math.max(1,p.bowlHeight), isTangent:true},
      ];
      if (p.serifLength > 0) {
        list.push({ id:'serifLength', anchor:{x:gg.left.x,y:0}, control:{x:gg.left.x-p.serifLength,y:0},
          paramName:'serifLength', deltaFromDrag:(dx)=>-dx });
      }
      return list;
    }
    function bounds(p) {
      const hs = p.strokeWeight/2;
      const serifPad = p.serifLength / 2;
      return { minX:-hs-serifPad, maxX:p.strokeWeight + 2*p.bowlWidth+serifPad, minY:-2*p.bowlHeight-hs, maxY:hs };
    }
    return {
      character:'e', defaultParams:dp, paramRanges:r,
      tangentParams:['bowlTopTension','bowlSideTension'],
      construct, handles,
  ...
  // ─── 't' ──────────────────────────────────────────────────────────────
  const t = (function () {
    const dp = { totalHeight:160, xHeight:140, capOvershoot:2, strokeWeight:24, crossbarLeft:18, crossbarRight:22, footCurl:10, footArm:8 };
    const r = {
      totalHeight:{min:100,max:220}, xHeight:{min:80,max:200}, capOvershoot:{min:0,max:12},
      strokeWeight:{min:6,max:44},
      crossbarLeft:{min:4,max:40}, crossbarRight:{min:4,max:40}, footCurl:{min:0,max:30}, footArm:{min:2,max:20},
    };
    function g(p) {
      const hs = p.strokeWeight/2;
      return { hs, stemX: hs+p.crossbarLeft, stemTopY:-p.totalHeight-(p.capOvershoot||0), crossbarY:-p.xHeight };
    }
    function construct(p) {
      const gg = g(p);
  ...
  // ─── 'l' ──────────────────────────────────────────────────────────────
  const l = (function () {
    const dp = { xHeight:140, ascenderRise:50, capOvershoot:2, strokeWeight:24, footCurl:16, footArm:10 };
    const r = { xHeight:{min:80,max:200}, ascenderRise:{min:10,max:100}, capOvershoot:{min:0,max:12}, strokeWeight:{min:6,max:44}, footCurl:{min:0,max:42}, footArm:{min:2,max:28} };
    function g(p) {
      const hs = p.strokeWeight/2;
      return { hs, stemX:hs, stemTopY:-p.xHeight-p.ascenderRise-(p.capOvershoot||0) };
    }
    function construct(p) {
      const gg = g(p);
      let stem = `M ${gg.stemX} ${gg.stemTopY} L ${gg.stemX} 0`;
  ...
  const b = (function () {
    const dp = {
      xHeight:140, bowlWidth:52, bowlHeight:54, strokeWeight:24,
      bowlTopTension:0.5523, bowlSideTension:0.5523, shoulder:36,
      serifLength:0, leftBearing:0, rightBearing:0,
    };
    const r = {
      xHeight:{min:80,max:200}, bowlWidth:{min:25,max:90}, bowlHeight:{min:25,max:90},
      strokeWeight:{min:6,max:44}, shoulder:{min:10,max:80},
      bowlTopTension:{min:0.15,max:1.4}, bowlSideTension:{min:0.15,max:1.4},
      serifLength: SERIF_RANGE, leftBearing: BEARING_RANGE, rightBearing: BEARING_RANGE,
    };
    function g(p) {
      const hs = p.strokeWeight/2, stemX = hs, bowlRight = stemX + p.bowlWidth * 2;
      const cy = -p.xHeight + p.shoulder;
      return { hs, stemX, bowlRight, cy,
        stemTopY:-p.xHeight,
    ...
    function construct(p) {
      ...
      if (p.serifLength > 0) {
        appendSerifStubs(paths, [{ x: gg.stemX, y: 0, len: p.serifLength, side: 'both' }]);
      }
      return paths;
    }
    function handles(p) {
      ...
      return [
        {id:'xHeight', anchor:{x:gg.stemX,y:gg.stemTopY}, control:{x:gg.stemX-18,y:gg.stemTopY}, paramName:'xHeight', deltaFromDrag:(_,dy)=>-dy},
        {id:'bowlWidth', anchor:gg.right, control:{x:gg.right.x+18,y:gg.right.y}, paramName:'bowlWidth', deltaFromDrag:(dx)=>dx},
        {id:'bowlHeight', anchor:gg.top, control:{x:gg.top.x,y:gg.top.y-18}, paramName:'bowlHeight', deltaFromDrag:(_,dy)=>-dy/2},
        {id:'shoulder', anchor:gg.left, control:{x:gg.left.x-18,y:gg.left.y}, paramName:'shoulder', deltaFromDrag:(_,dy)=>dy},
        {id:'strokeWeight', anchor:{x:gg.stemX,y:gg.cy/2}, control:{x:gg.stemX+p.strokeWeight/2+6,y:gg.cy/2}, paramName:'strokeWeight', deltaFromDrag:(dx)=>2*dx},
        {id:'bowlTopTension', anchor:gg.top, control:{x:gg.top.x+tw,y:gg.top.y}, paramName:'bowlTopTension', deltaFromDrag:(dx)=>dx/Math.max(1,p.bowlWidth), isTangent:true},
        {id:'bowlSideTension', anchor:gg.right, control:{x:gg.right.x,y:gg.right.y+th}, paramName:'bowlSideTension', deltaFromDrag:(_,dy)=>dy/Math.max(1,p.bowlHeight), isTangent:true},
      ];
    }
    function bounds(p) {
      const hs = p.strokeWeight/2;
      const gg = g(p);
      return { minX:-hs, maxX:gg.bowlRight+hs, minY:gg.stemTopY-hs, maxY:hs };
    }
    return {
      character:'b', defaultParams:dp, paramRanges:r,
      tangentParams:['bowlTopTension','bowlSideTension'],
      construct, handles,
      advance:(p)=>advanceWithBearings(p, p.strokeWeight + 2*p.bowlWidth),
      bounds,
  ...
  const c = (function () {
    const dp = {
      bowlWidth:54, bowlHeight:54, strokeWeight:24,
      bowlTopTension:0.5523, bowlSideTension:0.5523, opening:22,
      serifLength:0, leftBearing:0, rightBearing:0,
    };
    const r = {
      bowlWidth:{min:25,max:90}, bowlHeight:{min:25,max:90}, strokeWeight:{min:6,max:44},
      opening:{min:8,max:50}, bowlTopTension:{min:0.15,max:1.4}, bowlSideTension:{min:0.15,max:1.4},
      serifLength: SERIF_RANGE, leftBearing: BEARING_RANGE, rightBearing: BEARING_RANGE,
    };
    function g(p) {
      const hs = p.strokeWeight/2, cx = hs+p.bowlWidth, cy = -p.bowlHeight;
    ...
    function construct(p) {
      ...
      if (p.serifLength > 0) {
        appendSerifStubs(paths, [{ x: gg.left.x, y: 0, len: p.serifLength * 0.7, side: 'left' }]);
      }
      return paths;
    }
    function handles(p) {
      ...
      return [
        {id:'bowlWidth', anchor:gg.left, control:{x:gg.left.x-18,y:gg.left.y}, paramName:'bowlWidth', deltaFromDrag:(dx)=>-dx},
        {id:'bowlHeight', anchor:gg.top, control:{x:gg.top.x,y:gg.top.y-18}, paramName:'bowlHeight', deltaFromDrag:(_,dy)=>-dy/2},
        {id:'opening', anchor:gg.openTop, control:{x:gg.openTop.x,y:gg.openTop.y-p.opening}, paramName:'opening', deltaFromDrag:(_,dy)=>-dy},
        {id:'strokeWeight', anchor:{x:gg.cx,y:gg.cy}, control:{x:gg.cx+p.strokeWeight/2+6,y:gg.cy}, paramName:'strokeWeight', deltaFromDrag:(dx)=>2*dx},
        {id:'bowlTopTension', anchor:gg.top, control:{x:gg.top.x+tw,y:gg.top.y}, paramName:'bowlTopTension', deltaFromDrag:(dx)=>dx/Math.max(1,p.bowlWidth), isTangent:true},
        {id:'bowlSideTension', anchor:gg.left, control:{x:gg.left.x,y:gg.left.y+th}, paramName:'bowlSideTension', deltaFromDrag:(_,dy)=>dy/Math.max(1,p.bowlHeight), isTangent:true},
      ];
    }
    function bounds(p) {
      const hs = p.strokeWeight/2;
      return { minX:-hs, maxX:p.strokeWeight+2*p.bowlWidth+hs, minY:-2*p.bowlHeight-hs, maxY:hs };
    }
    return {
      character:'c', defaultParams:dp, paramRanges:r,
      tangentParams:['bowlTopTension','bowlSideTension'],
      construct, handles,
      advance:(p)=>advanceWithBearings(p, p.strokeWeight + 2*p.bowlWidth),
      bounds,
  ...
  const m = (function () {
    const dp = {
      xHeight:140, archWidth:44, strokeWeight:24, shoulder:34, archTension:0.5523,
      serifLength:0, leftBearing:0, rightBearing:0,
    };
    const r = {
      xHeight:{min:80,max:200}, archWidth:{min:28,max:70}, strokeWeight:{min:6,max:44},
      shoulder:{min:10,max:70}, archTension:{min:0.15,max:1.4},
      serifLength: SERIF_RANGE, leftBearing: BEARING_RANGE, rightBearing: BEARING_RANGE,
    };
    function g(p) {
      const hs = p.strokeWeight/2;
      const leftX = hs, midX = hs + p.archWidth, rightX = hs + 2*p.archWidth;
      const xLineY = -p.xHeight, shoulderY = xLineY + p.shoulder;
    ...
    function handles(p) {
      ...
      return [
        {id:'xHeight', anchor:{x:midArch,y:gg.xLineY}, control:{x:midArch,y:gg.xLineY-18}, paramName:'xHeight', deltaFromDrag:(_,dy)=>-dy},
        {id:'archWidth', anchor:{x:gg.rightX,y:gg.shoulderY/2}, control:{x:gg.rightX+18,y:gg.shoulderY/2}, paramName:'archWidth', deltaFromDrag:(dx)=>dx/2},
        {id:'shoulder', anchor:{x:gg.leftX,y:gg.shoulderY}, control:{x:gg.leftX-18,y:gg.shoulderY}, paramName:'shoulder', deltaFromDrag:(_,dy)=>dy},
        {id:'strokeWeight', anchor:{x:gg.midX,y:gg.shoulderY/2}, control:{x:gg.midX+p.strokeWeight/2+6,y:gg.shoulderY/2}, paramName:'strokeWeight', deltaFromDrag:(dx)=>2*dx},
        {id:'archTension', anchor:{x:midArch,y:gg.xLineY}, control:{x:midArch+tArm,y:gg.xLineY}, paramName:'archTension', deltaFromDrag:(dx)=>dx/Math.max(1,p.archWidth*0.5), isTangent:true},
      ];
    }
    function bounds(p) {
      const gg = g(p), hs = p.strokeWeight/2;
      return { minX:-hs, maxX:gg.rightX+hs, minY:gg.xLineY-hs, maxY:hs };
    }
    return {
      character:'m', defaultParams:dp, paramRanges:r, tangentParams:['archTension'],
      construct, handles,
  ...
  const g = (function () {
    const dp = {
      xHeight:140, bowlWidth:52, bowlHeight:50, strokeWeight:24,
      bowlTopTension:0.5523, bowlSideTension:0.5523, descenderDrop:48, descenderCurl:14,
      serifLength:0, leftBearing:0, rightBearing:0,
    };
    const r = {
      xHeight:{min:80,max:200}, bowlWidth:{min:25,max:90}, bowlHeight:{min:22,max:80},
      strokeWeight:{min:6,max:44}, descenderDrop:{min:20,max:80}, descenderCurl:{min:0,max:32},
      bowlTopTension:{min:0.15,max:1.4}, bowlSideTension:{min:0.15,max:1.4},
      serifLength: SERIF_RANGE, leftBearing: BEARING_RANGE, rightBearing: BEARING_RANGE,
    };
    function gGeom(p) {
      ...
      return { hs, cx, cy, tailX,
        top:{x:cx,y:cy-p.bowlHeight}, right:{x:cx+p.bowlWidth,y:cy},
        bottom:{x:cx,y:cy+p.bowlHeight}, left:{x:cx-p.bowlWidth,y:cy},
    ...
    function construct(p) {
      ...
      paths.push(`M ${gg.right.x} ${gg.bottom.y} L ${gg.tailX} ${p.descenderDrop} C ${gg.tailX+p.descenderCurl*0.4} ${p.descenderDrop+p.descenderCurl*0.2}, ${gg.tailEnd.x} ${gg.tailEnd.y}, ${gg.tailEnd.x} ${gg.tailEnd.y}`);
      if (p.serifLength > 0) {
        appendSerifStubs(paths, [{ x: gg.left.x, y: 0, len: p.serifLength * 0.65, side: 'left' }]);
      }
      return paths;
    }
    function handles(p) {
      const gg = gGeom(p), tw = p.bowlWidth*p.bowlTopTension, th = p.bowlHeight*p.bowlSideTension;
      return [
        {id:'bowlWidth', anchor:gg.left, control:{x:gg.left.x-18,y:gg.left.y}, paramName:'bowlWidth', deltaFromDrag:(dx)=>-dx},
        {id:'bowlHeight', anchor:gg.top, control:{x:gg.top.x,y:gg.top.y-18}, paramName:'bowlHeight', deltaFromDrag:(_,dy)=>-dy/2},
        {id:'descenderDrop', anchor:{x:gg.tailX,y:gg.bottom.y}, control:{x:gg.tailX,y:p.descenderDrop}, paramName:'descenderDrop', deltaFromDrag:(_,dy)=>dy},
        {id:'descenderCurl', anchor:gg.tailEnd, control:{x:gg.tailEnd.x+p.descenderCurl,y:gg.tailEnd.y}, paramName:'descenderCurl', deltaFromDrag:(dx)=>dx},
        {id:'strokeWeight', anchor:gg.right, control:{x:gg.right.x+p.strokeWeight/2+6,y:gg.right.y}, paramName:'strokeWeight', deltaFromDrag:(dx)=>2*dx},
        {id:'bowlTopTension', anchor:gg.top, control:{x:gg.top.x+tw,y:gg.top.y}, paramName:'bowlTopTension', deltaFromDrag:(dx)=>dx/Math.max(1,p.bowlWidth), isTangent:true},
        {id:'bowlSideTension', anchor:gg.right, control:{x:gg.right.x,y:gg.right.y+th}, paramName:'bowlSideTension', deltaFromDrag:(_,dy)=>dy/Math.max(1,p.bowlHeight), isTangent:true},
      ];
    ...
    function bounds(p) {
      const gg = gGeom(p), hs = p.strokeWeight/2;
      return { minX:-hs, maxX:gg.right.x+hs+Math.max(p.descenderCurl,8), minY:-2*p.bowlHeight-hs, maxY:p.descenderDrop+hs };
    }
    return {
      character:'g', defaultParams:dp, paramRanges:r,
      tangentParams:['bowlTopTension','bowlSideTension'],
      construct, handles,
      advance:(p)=>advanceWithBearings(p, p.strokeWeight + 2*p.bowlWidth + Math.max(0, p.descenderCurl*0.2)),
      bounds,
    };
  })();
  ...
  const CURATED_KEYS = ['a', 'n', 'o', 's', 'h', 'i', 'e', 't', 'r', 'l', 'w', 'd', 'b', 'c', 'm', 'g', ' '];
  ...
  const OUTLINE_DISCLAIMER_SHORT = 'Prototype only — reference outlines for experimentation, not a licensed font product.';
  ...
  function getOpentypeParser() {
    ...
    throw new Error('Outline mode requires opentype.js — call SculptLettering.setOpentypeParser(opentype) first.');
  }
  ...
  function outlineAttributionBlock(presetKey) {
    ...
    return [
      '<!-- ' + OUTLINE_DISCLAIMER + ' -->',
      '<!-- Font: ' + p.fontRef + ' | ' + p.license + ' | ' + p.copyright + ' -->',
    ].join('\n');
  }
  ...
  function outlineAttributionHtml(presetKey) {
    ...
    return (
      ...
      '<p><strong>Prototype / experimentation.</strong> ' + OUTLINE_DISCLAIMER_SHORT + '</p>' +
      '<p class="sculpt-outline-attribution">' + p.attribution + ' (' + p.license + ')</p>' +
      '</aside>'
    );
  }
  ...
  async function loadFontForPreset(presetKey) {
    ...
    if (!preset || !preset.fontUrl) {
      throw new Error("Preset '" + presetKey + "' has no fontUrl for outline mode.");
    }
    if (_fontCache[presetKey]) return _fontCache[presetKey];
    const ot = getOpentypeParser();
    const resp = await fetch(preset.fontUrl);
    if (!resp.ok) throw new Error('Failed to load font for ' + presetKey + ': HTTP ' + resp.status);
  ...
  function clonePathCommands(path) {
    ...
    return path.commands.map(function (cmd) {
      ...
      if (cmd.x1 != null) { c.x1 = cmd.x1; c.y1 = cmd.y1; }
      if (cmd.x2 != null) { c.x2 = cmd.x2; c.y2 = cmd.y2; }
      return c;
    });
  }
    ...
    for (const cmd of commands) {
      if (cmd.type === 'M') {
        ...
        for (let i = 1; i <= N; i++) {
          ...
          const mt = 1 - tt;
          points.push({
            x: mt * mt * mt * prev.x + 3 * mt * mt * tt * cmd.x1 + 3 * mt * tt * tt * cmd.x2 + tt * tt * tt * cmd.x,
            y: mt * mt * mt * prev.y + 3 * mt * mt * tt * cmd.y1 + 3 * mt * tt * tt * cmd.y2 + tt * tt * tt * cmd.y,
          });
        }
        prev = { x: cmd.x, y: cmd.y };
      }
   ...
   */
  function applyBubbliness(commands, t, _bounds) {
    ...
    const amplitude = glyphSize * 0.06 * Math.sqrt(t);
  ...
  function applyPresetAxesToCommands(commands, axisValues, preset) {
    if (!commands || !preset || !preset.axes) return { commands, serifExtra: '' };
  ...
  function buildDeformedPathData(baseGlyph, axisValues, preset) {
    ...
    const applied = applyPresetAxesToCommands(baseGlyph.baseCommands, axisValues, preset);
  ...
  function extractOutlineGlyph(font, character, fontSize) {
    ...
    const advance = (glyph.advanceWidth || font.unitsPerEm * 0.5) * (fontSize / font.unitsPerEm);
  ...
  const bubbly = {
    ...
    fontUrl: 'https://cdn.jsdelivr.net/npm/@fontsource/rubik-bubbles@5.2.5/files/rubik-bubbles-latin-400-normal.woff',
    license: 'OFL-1.1',
    copyright: 'Copyright 2022 The Rubik Bubbles Project Authors (https://github.com/googlefonts/rubik-bubbles)',
    ...
    glyphParams: curatedGlyphParams({
      ...
      b: { xHeight:140, bowlWidth:54, bowlHeight:56, strokeWeight:28, shoulder:36, bowlTopTension:0.62, bowlSideTension:0.62 },
      c: { bowlWidth:56, bowlHeight:58, strokeWeight:28, opening:22, bowlTopTension:0.62, bowlSideTension:0.62 },
      m: { xHeight:140, archWidth:44, strokeWeight:28, shoulder:34, archTension:0.62 },
      g: { xHeight:140, bowlWidth:52, bowlHeight:50, strokeWeight:28, descenderDrop:48, bowlTopTension:0.62, bowlSideTension:0.62 },
  ...
  const instrumentSerif = {
    ...
    fontUrl: 'https://cdn.jsdelivr.net/npm/@fontsource/instrument-serif@5.2.5/files/instrument-serif-latin-400-normal.woff',
    license: 'OFL-1.1',
    ...
    axes: [
      { id: 'serifLength', label: 'Serif length', min: 0, max: 1, default: 0.5, appliesTo: 'serif' },
    ],
    defaults: {
      strokeWeight: 22,
      xHeight: 138,
    ...
    glyphParams: curatedGlyphParams({
      a: { xHeight:138, bowlWidth:58, bowlHeight:64, strokeWeight:22, aperture:12, terminalLength:22, bowlTopTension:0.74, bowlBottomTension:0.74, terminalArm:14, serifLength:10 },
      ...
      e: { bowlWidth:60, bowlHeight:62, strokeWeight:22, aperture:30, crossbarOffset:10, bowlTopTension:0.72, bowlSideTension:0.72, serifLength:8 },
      ...
      l: { xHeight:138, ascenderRise:54, strokeWeight:22, footCurl:16, footArm:12, serifLength:8 },
      ...
      b: { xHeight:138, bowlWidth:54, bowlHeight:56, strokeWeight:22, shoulder:34, bowlTopTension:0.74, bowlSideTension:0.74, serifLength:10 },
      c: { bowlWidth:56, bowlHeight:58, strokeWeight:22, opening:20, bowlTopTension:0.74, bowlSideTension:0.74, serifLength:8 },
      m: { xHeight:138, archWidth:42, strokeWeight:22, shoulder:32, archTension:0.7, serifLength:9 },
      g: { xHeight:138, bowlWidth:50, bowlHeight:48, strokeWeight:22, descenderDrop:46, bowlTopTension:0.74, bowlSideTension:0.74, serifLength:8 },
  ...
  const sourceSans = {
    ...
    fontUrl: 'https://cdn.jsdelivr.net/npm/@fontsource/source-sans-3@5.2.5/files/source-sans-3-latin-400-normal.woff',
    license: 'OFL-1.1',
    copyright: 'Copyright 2023 The Source Sans 3 Project Authors (https://github.com/adobe-fonts/source-sans)',
    attribution: 'Source Sans 3 by Paul D. Hunt, Adobe',
    strokeJoin: 'miter',
    axes: [
      { id: 'width', label: 'Width', min: 0, max: 1, default: 0.5 },
    ],
    defaults: {
      strokeWeight: 24,
      xHeight: 140,
    ...
    glyphParams: curatedGlyphParams({
      ...
      b: { xHeight:140, bowlWidth:54, bowlHeight:56, strokeWeight:24, shoulder:36, bowlTopTension:0.55, bowlSideTension:0.55 },
      c: { bowlWidth:56, bowlHeight:56, strokeWeight:24, opening:22, bowlTopTension:0.55, bowlSideTension:0.55 },
      m: { xHeight:140, archWidth:44, strokeWeight:24, shoulder:34, archTension:0.55 },
      g: { xHeight:140, bowlWidth:52, bowlHeight:50, strokeWeight:24, descenderDrop:48, bowlTopTension:0.55, bowlSideTension:0.55 },
  ...
  const bitter = {
    name: 'bitter',
    fontRef: 'Bitter',
    fontUrl: 'https://cdn.jsdelivr.net/npm/@fontsource/bitter@5.2.5/files/bitter-latin-400-normal.woff',
    license: 'OFL-1.1',
    copyright: 'Copyright 2011 The Bitter Project Authors (https://github.com/solmatas/Bitter-Pro)',
    attribution: 'Bitter by Huerta Tipográfica',
    strokeJoin: 'miter',
    axes: [
      { id: 'serifLength', label: 'Serif length', min: 0, max: 1, default: 0.55, appliesTo: 'serif' },
    ],
    defaults: {
      strokeWeight: 28,
      xHeight: 132,
    ...
    glyphParams: curatedGlyphParams({
      a: { xHeight:132, bowlWidth:62, bowlHeight:58, strokeWeight:28, aperture:10, terminalLength:18, bowlTopTension:0.38, bowlBottomTension:0.38, terminalArm:10, serifLength:12 },
      ...
      e: { bowlWidth:62, bowlHeight:58, strokeWeight:28, aperture:24, crossbarOffset:10, bowlTopTension:0.38, bowlSideTension:0.38, serifLength:10 },
      ...
      l: { xHeight:132, ascenderRise:48, strokeWeight:28, footCurl:14, footArm:8, serifLength:10 },
      ...
      b: { xHeight:132, bowlWidth:56, bowlHeight:54, strokeWeight:28, shoulder:38, bowlTopTension:0.38, bowlSideTension:0.38, serifLength:12 },
      c: { bowlWidth:58, bowlHeight:56, strokeWeight:28, opening:20, bowlTopTension:0.38, bowlSideTension:0.38, serifLength:10 },
      m: { xHeight:132, archWidth:44, strokeWeight:28, shoulder:36, archTension:0.38, serifLength:11 },
      g: { xHeight:132, bowlWidth:52, bowlHeight:48, strokeWeight:28, descenderDrop:46, bowlTopTension:0.38, bowlSideTension:0.38, serifLength:10 },
  ...
  const ibmPlexMono = {
    ...
    fontUrl: 'https://cdn.jsdelivr.net/npm/@fontsource/ibm-plex-mono@5.2.5/files/ibm-plex-mono-latin-400-normal.woff',
    license: 'OFL-1.1',
    ...
    glyphParams: curatedGlyphParams({
      ...
      d: { xHeight:128, ascenderRise:44, bowlWidth:52, bowlHeight:54, strokeWeight:22, bowlTopTension:0.5, bowlSideTension:0.5, rightBearing: 4 },
      b: { xHeight:128, bowlWidth:48, bowlHeight:50, strokeWeight:22, shoulder:32, bowlTopTension:0.5, bowlSideTension:0.5, rightBearing: 6 },
      c: { bowlWidth:50, bowlHeight:50, strokeWeight:22, opening:20, bowlTopTension:0.5, bowlSideTension:0.5, rightBearing: 8 },
      m: { xHeight:128, archWidth:40, strokeWeight:22, shoulder:30, archTension:0.5, rightBearing: 4 },
      g: { xHeight:128, bowlWidth:48, bowlHeight:46, strokeWeight:22, descenderDrop:44, bowlTopTension:0.5, bowlSideTension:0.5, rightBearing: 6 },
  ...
  class Wordmark {
    ...
    _resetGlyphParams(glyph) {
      const presetParams = resolvePresetParams(this._preset, glyph.character, glyph.module) || {};
      glyph.params = Object.assign({}, glyph.module.defaultParams, presetParams);
      glyph._clampAll();
    }
    ...
    setPreset(preset) {
      ...
      if (this._mouseFollow) {
        this._mouseFollow.restParams = this.glyphs.map((g) => Object.assign({}, g.params));
      }
      this._render();
    }
    ...
    toSVG() {
      ...
      const header = '<!-- Parametric sculpt-lettering export — prototype learning toy. -->\n';
      return header +
        '<svg xmlns="' + SVG_NS + '" viewBox="0 0 ' + totalWidth + ' ' + height + '" width="' + totalWidth + '" height="' + height + '">' +
        body + '</svg>';
    }
  ...
  class DeformableOutlineWordmark {
    ...
    async _loadGlyphs(text) {
      ...
      for (const ch of chars) {
        try {
          const base = await getOutlineGlyphData(this.presetKey, ch, this.fontSize);
          const _baseGlyph = {
            character: ch,
          ...
          const pathData = buildDeformedPathData(_baseGlyph, this.axisValues, preset);
          const applied = base.baseCommands
            ? applyPresetAxesToCommands(base.baseCommands, this.axisValues, preset)
            : null;
    ...
    _applyDeformToGlyphs() {
      ...
      for (const g of this.glyphs) {
        ...
        g.pathData = buildDeformedPathData(g._baseGlyph, this.axisValues, preset);
        const applied = g._baseGlyph.baseCommands
          ? applyPresetAxesToCommands(g._baseGlyph.baseCommands, this.axisValues, preset)
          : null;
        g.bounds = applied
          ? boundsFromCommands(applied.commands)
    ...
    _axisHandleDefs(layout) {
      ...
      const handles = [];
      axes.forEach((ax, i) => {
        const v = this.axisValues[ax.id] != null ? this.axisValues[ax.id] : ax.default;
    ...
    _render() {
      ...
      for (const pg of placed) {
        if (pg.glyph.fallback === 'parametric' && pg.glyph.fallbackPaths) {
          const paths = pg.glyph.fallbackPaths.map((d) =>
            '<path d="' + d + '" stroke="' + this.color + '" stroke-width="24" fill="none" stroke-linecap="round" stroke-linejoin="round"/>'
          ).join('');
          parts.push('<g transform="translate(' + pg.x + ',' + baselineY + ')">' + paths + '</g>');
        } else {
          parts.push(
            '<g transform="translate(' + pg.x + ',' + baselineY + ')">' +
            '<path d="' + pg.glyph.pathData + '" fill="' + this.color + '" fill-rule="evenodd" stroke="none"/>' +
            '</g>'
          );
        }
      }
      ...
      if (this.interactive && this.handleLayer) {
        ...
        axisHandles.forEach((h) => {
          html.push('<line x1="' + h.anchor.x + '" y1="' + h.anchor.y + '" x2="' + h.control.x + '" y2="' + h.control.y + '" stroke="#1a2f6e" stroke-width="1" opacity="0.55" pointer-events="none"/>');
          html.push('<circle data-axis-handle="true" data-handle-id="' + h.id + '" cx="' + h.control.x + '" cy="' + h.control.y + '" r="' + targetRadius + '" fill="#000" fill-opacity="0.001" stroke="none" cursor="grab" style="touch-action:none"/>');
          html.push('<circle cx="' + h.control.x + '" cy="' + h.control.y + '" r="' + controlRadius + '" fill="#ffffff" stroke="#1a2f6e" stroke-width="1.5" pointer-events="none"/>');
        });
        this.handleLayer.innerHTML = html.join('');
        this.handleLayer.querySelectorAll('circle[data-axis-handle]').forEach((c) => {
          c.addEventListener('pointerdown', (ev) => this._onDragStart(ev));
          c.addEventListener('pointerenter', (ev) => this._onHandleEnter(ev));
          c.addEventListener('pointerleave', () => this._onHandleLeave());
        });
    ...
    _renderTooltip(totalWidth, height, layout) {
      if (!this.tooltipLayer) return;
      if (!this.interactive || !this.tooltipState) { this.tooltipLayer.innerHTML = ''; return; }
      const h = this._axisHandleDefs(layout).find((x) => x.id === this.tooltipState.handleId);
      if (!h) { this.tooltipLayer.innerHTML = ''; return; }
      ...
      this.tooltipLayer.innerHTML =
        '<g pointer-events="none"><rect x="' + bx + '" y="' + by + '" width="' + bubbleW + '" height="' + bubbleH + '" rx="3" fill="#ffffff" stroke="#1a2f6e" stroke-width="1"/>' +
        '<text x="' + (bx + pad) + '" y="' + (by + 15) + '" fill="#525860" font-size="11.5" font-family="ui-monospace, monospace">' + this._esc(labelText) + '</text>' +
        '<text x="' + (bx + pad + labelW + gap) + '" y="' + (by + 15) + '" fill="#1a2f6e" font-size="11.5" font-family="ui-monospace, monospace" font-weight="600">' + this._esc(valueText) + '</text></g>';
    }

    _onHandleEnter(e) {
      if (this.dragState) return;
      this.tooltipState = { handleId: e.currentTarget.dataset.handleId, pinned: false };
      const layout = this._layoutCache || this._layout();
      this._renderTooltip(layout.totalWidth, this.padding + layout.maxAscent + layout.maxDescent + this.padding, layout);
    }
    _onHandleLeave() {
      if (this.dragState || (this.tooltipState && this.tooltipState.pinned)) return;
      this.tooltipState = null;
    ...
    _onMouseFollowMove(e) {
      if (!this._mouseFollow) return;
      const o = this._mouseFollow.opts;
      let dx = o.origin === 'topleft' ? e.clientX : e.clientX - window.innerWidth / 2;
      dx *= o.strength;
      const c = o.clamp;
      if (c > 0) { if (dx > c) dx = c; else if (dx < -c) dx = -c; }
      this._applyMouseFollow(dx, 0);
    }
    ...
    _applyMouseFollow(dx) {
      if (!this._mouseFollow) return;
      const o = this._mouseFollow.opts;
      const axisId = o.mapAxis || (this.presetAxes[0] && this.presetAxes[0].id);
      if (!axisId) return;
      const ax = this.presetAxes.find((a) => a.id === axisId);
      if (!ax) return;
      const rest = this._mouseFollow.restAxes[axisId];
      const span = ax.max - ax.min;
      const t = Math.max(ax.min, Math.min(ax.max, rest + (dx / Math.max(1, o.clamp)) * span * 0.5));
      this.axisValues[axisId] = t;
      this._applyDeformToGlyphs();
      this._render();
    }
    ...
    toSVG() {
      ...
      return outlineAttributionBlock(this.presetKey) + '\n' +
        '<svg xmlns="' + SVG_NS + '" viewBox="0 0 ' + totalWidth + ' ' + height + '" width="' + totalWidth + '" height="' + height + '" data-render-mode="outline">' +
        body + '</svg>';
    }
    ...
    static async fromState(state) {
      ...
      if (state.axisValues) wm.axisValues = Object.assign(wm.axisValues, state.axisValues);
...
(function () {
  ...
  class OutlineWordmark {
    ...
    async _loadGlyphs(text) {
      ...
      const loaded = [];
      for (const ch of chars) {
        loaded.push(await getOutlineGlyphData(this.presetKey, ch, this.fontSize));
      }
      this.glyphs = loaded;
      this.text = text;
    }
...
(function () {
  ...
  async function createWordmark(text, options) {
    ...
    const presetKey = options.presetKey || (options.preset && options.preset.name) || options.preset;
    if (mode === 'outline-static' && presetKey && presetKey !== 'none') {
      return OutlineWordmark.create(text, Object.assign({}, options, { presetKey: presetKey }));
    }
    if (mode === 'outline' && presetKey && presetKey !== 'none') {
      return DeformableOutlineWordmark.create(text, Object.assign({}, options, { presetKey: presetKey }));
    }
    const presetObj = typeof options.preset === 'string'
      ? presets[options.preset]
      : options.preset;
    return new Wordmark(text, Object.assign({}, options, { preset: presetObj }));
  }
  ...
  const glyphs = Object.assign({
    a, n, o, s, h, i: i_, e, t, r: r_, l, w, d, b, c, m, g, space,
  }, extraLowercaseGlyphs, uppercaseGlyphs);

  return {
    Wordmark, OutlineWordmark, DeformableOutlineWordmark, Glyph,
    registerGlyph, getRegisteredGlyphs,
    createWordmark, setOpentypeParser,
    OUTLINE_DISCLAIMER, OUTLINE_DISCLAIMER_SHORT,
    outlineAttributionHtml, outlineAttributionBlock,
    buildDeformedPathData, defaultAxisValuesForPreset,
    glyphs,
    presets: presets,
  };
});
