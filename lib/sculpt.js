...
(function (root, factory) {
   ...
   * `weight` (Brief 9 #5) — the half-width the upcoming dilation will eat from
   * the stroke; reserved in the growth floor so the ring holds with weight on.
   */
  function counterScale(commands, s, center, weight) {
    ...
    const c = center || counterCentroid(commands);
    if (!c) return commands;
    const w = weight || 0;

    let sEff = s;
    if (s > 1) {
      // Outer contour = largest-|area| subpath.
      let outerI = 0;
      let outerA = 0;
      for (let i = 0; i < subs.length; i++) {
        const a = Math.abs(signedAreaOfCommands(subs[i].commands));
      ...
      // reserves `weight/2` — the half-width the later dilation eats — so the
      // ring never breaks across the full 0.6–1.4 range with weight engaged.
...
(function () {
  ...
  class AnatomyDeformWordmark {
    ...
    _deformedEdgePoints(g) {
      ...
      // fraction, and widen it downward by `weight/2`: dilation pushes feet
      // below the baseline by half the stroke, so a strict |y|≤tol band loses
      // them under heavy weight. Feet are at/just-below the baseline (y ≥ 0 in
      // the y-down outline space), so the band is [−tol, tol + weight/2].
      const xH =
